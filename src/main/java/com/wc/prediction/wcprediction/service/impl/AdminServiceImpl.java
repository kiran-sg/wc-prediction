...
public class AdminServiceImpl implements AdminService {
    ...
    private int calculatePoints(Prediction prediction, MatchResult result) {
        ...
        if (prediction.getFirstGoalscorerPredicted().equals(result.getFirstGoalscorer())) {
            points += 5;
        }
        if (prediction.getWinningGoalscorerPredicted().equals(result.getWinningGoalscorer())) {
            points += 5;
        }
        if (prediction.getPlayerOfMatchPredicted().equals(result.getPlayerOfMatch())) {
            points += 3;
        }
